...
function displayGameDetails(game, history) {
    ...
    modalContent.innerHTML = `
        ...
        <div class="line-summary">
            <div class="line-item">
                <div class="line-title">ספרד נוכחי</div>
                <div class="line-value">${game.live_spread !== null ? game.live_spread : '-'}</div>
            </div>
            <div class="line-item">
                <div class="line-title">טוטאל נוכחי</div>
                <div class="line-value">${game.live_total !== null ? game.live_total : '-'}</div>
            </div>
            <div class="line-item">
                <div class="line-title">שינוי ספרד</div>
                <div class="line-value ${spreadDiff > 0 ? 'green' : spreadDiff < 0 ? 'red' : ''}">
                    ${spreadDiff > 0 ? '+' : ''}${spreadDiff.toFixed(1)}
                </div>
            </div>
            <div class="line-item">
                <div class="line-title">שינוי טוטאל</div>
                <div class="line-value ${totalDiff > 0 ? 'green' : totalDiff < 0 ? 'red' : ''}">
...
function updateStats(data) {
    statsElements.opportunityPercentage.textContent = `${data.opportunity_percentage}%`;
    statsElements.redOpportunities.textContent = data.red_opportunities;
    statsElements.greenOpportunities.textContent = data.green_opportunities;
    statsElements.blueOpportunities.textContent = data.blue_opportunities;
    statsElements.totalLive.textContent = data.total_live;
    statsElements.totalUpcoming.textContent = data.total_upcoming;
}
...
function updateLastRefreshTime() {
    const now = new Date();
    const timeStr = now.toLocaleTimeString('he-IL');
    lastUpdateTimeElement.textContent = `עודכן לאחרונה: ${timeStr}`;
}
